...
function RecordCard({ record, poster, onOpen, onEdit, onDelete, onStatus, onLock }: { record: WatchRecord; poster?: boolean; onOpen: () => void; onEdit: () => void; onDelete: () => void; onStatus: (status: Status) => void; onLock: () => void }) {
  ...
  return <article className={`mobile-record-card ${poster ? 'mobile-poster-card' : ''}`} data-record-id={record.id}>
    <button type="button" className="mobile-record-open" onClick={onOpen} aria-label={`打开 ${title}`}>
      ...
      <div className="mobile-record-card-body"><div className="mobile-record-title">{title}</div>{displayTitlesOf(record).secondary && <div className="mobile-record-subtitle">{displayTitlesOf(record).secondary}</div>}<div className="mobile-record-meta"><span>{record.mediaType}</span><span>{record.status}</span>{record.releaseYear && <span>{record.releaseYear}</span>}{record.rating != null && <span>★ {record.rating}</span>}{record.isLocked && <span>🔒 已锁定</span>}</div></div>
    </button>
...
function DetailPage({ record, onBack, onEdit, onDelete, onLock, onStatus }: { record: WatchRecord; onBack: () => void; onEdit: () => void; onDelete: () => void; onLock: () => void; onStatus: (status: Status) => void }) {
  const title = titleOf(record);
...
export default function MobileLibraryPage(props: MobileLibraryPageProps) {
  const { records, detailId, formMode, onDetail, onForm, onBack, onAdd, onUpdate, onDelete, onLock, onStatus, onNotify } = props;
  ...
  if (props.route === 'detail' && detailId && detail) return <DetailPage record={detail} onBack={onBack} onEdit={() => { if (!detail.isLocked) onForm('edit', detail); else onNotify('warning', '已锁定记录不能编辑。'); }} onDelete={() => void onDelete(detail)} onLock={() => void onLock(detail, !detail.isLocked)} onStatus={status => void onStatus(detail, status)} />;
  const emptyKind = records.length === 0 ? 'library' : filtered.length === 0 ? (search ? 'search' : 'filter') : null;
  return <section className="mobile-library" aria-labelledby="mobile-library-title" ref={contentRef}><LibraryToolbar search={searchInput} setSearch={value => { setSearchInput(value); if (!value) setSearch(''); }} prefs={prefs} setPrefs={setPrefs} onFilter={() => setFilterOpen(true)} filterTriggerRef={filterTriggerRef} /><div className="mobile-count-row"><span>{filtered.length} / {records.length} 条</span></div>{emptyKind ? <EmptyState kind={emptyKind} onAdd={() => onForm('new')} /> : prefs.viewMode === 'poster' ? <div className="mobile-poster-grid">{filtered.map(record => <RecordCard key={record.id} record={record} poster onOpen={() => { captureScroll(); onDetail(record); }} onEdit={() => { captureScroll(); onForm('edit', record); }} onDelete={() => void onDelete(record)} onStatus={status => void onStatus(record, status)} onLock={() => void onLock(record, !record.isLocked)} />)}</div> : <div className="mobile-record-list">{filtered.map(record => <RecordCard key={record.id} record={record} onOpen={() => { captureScroll(); onDetail(record); }} onEdit={() => { captureScroll(); onForm('edit', record); }} onDelete={() => void onDelete(record)} onStatus={status => void onStatus(record, status)} onLock={() => void onLock(record, !record.isLocked)} />)}</div>}{filterOpen && <FilterSheet prefs={prefs} setPrefs={setPrefs} records={records} onClose={() => setFilterOpen(false)} returnFocusRef={filterTriggerRef} />}</section>;
}
